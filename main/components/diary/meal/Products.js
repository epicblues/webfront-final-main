...
console.log(res1,res2);
  ...
  const handleRecipeModal = (index) => setRecipeOpen(state => {
    const newState = [...state];
    newState[index] = !newState[index];
    return newState
  })
  ...
  return (
    <>
      ...
      <div style={{padding: '10px'}}>
        <div className="ui middle aligned selection list" style={{textAlign: 'left'}}>
          <i className='book icon'></i>레시피
          {filteredRecipeData.length !== 0 && filteredRecipeData.map((value, index) => {
            return (
              // 레시피 검색 리스트 출력
              <RecipeModal
                value={value}
